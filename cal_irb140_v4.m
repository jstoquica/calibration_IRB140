...
%conceito de sub-regiões. Por sub-região.
%%
...
% Validação de singularidades baseado nos limites das juntas
...
% Funções do IRB140:
% função que gera os erros de posicionamento 
[Mx My Mz M] = fk_irb140_med(Theta);
% função que cria a cinemática direta com/sem variação
...
% Quantificar erros: média, max e desvio padrão
desv = std(eM);
EmaxM = max(eM);
prom(it_s) = mean(eM);
...
promNewX(it_s) = mean(eMnewX);
promNewY(it_s) = mean(eMnewY);
promNewZ(it_s) = mean(eMnewZ);
...
%% Gerar tabela com os parametros de erro 
